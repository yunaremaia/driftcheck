...
DRIFT_RULES = {
    ...
    "package_lock_drifts": (
        ...
        "package-lock Integrity Drift",
        "A package-lock.json resolution is missing, invalid, or outside its package.json semver range",
    ),
    "engines_drifts": (
        "engines-drift",
        "Engines Drift",
...
def _drift_message(drift_type: str, d: dict) -> str:
    ...
    elif drift_type == "package_lock_drifts":
        return d.get("detail", "package-lock integrity drift detected")
...
def to_sarif(result: dict, version: str | None = None, root: Path | None = None) -> dict:
    ...
    drift_keys = [
        ...
        "external_resource_drifts", "dependabot_drifts", "lockfile_drifts", "package_lock_drifts",
        "engines_drifts",
